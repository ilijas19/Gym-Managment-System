...
const trainerLogin = async (req, res) => {
  ...
  if (!trainer) {
    throw new CustomError.NotFoundError(`No trainer with username ${username}`);
  }
  const isPasswordCorrect = await trainer.comparePassword(password);
  if (!isPasswordCorrect) {
    throw new CustomError.UnauthenticatedError("Wrong password");
  }
  const tokenUser = createTokenUser(trainer);
  const existingToken = await Token.findOne({ user: trainer._id });
  if (existingToken) {
    const { isValid } = existingToken;
    if (!isValid) {
      throw new CustomError.UnauthenticatedError("Auhtentication failed");
  ...
  const ip = req.ip;
  const userAgent = req.headers["user-agent"];
  await Token.create({ refreshToken, ip, userAgent, user: trainer._id });
  attachCookiesToResponse({ res, user: tokenUser, refreshToken });
  res.status(StatusCodes.OK).json({ tokenUser });
};
